...
void initial_3D(hydro_fields f, hydro_params p) {
  ...
  double surface_tension = 2.0*sqrt(2.0)/81.0*p.alpha*p.alpha*p.alpha
  ...
  double R_critical = 2.0*surface_tension/(-1.0*Vf(p, p.Tconst, phimin));

  double R_scaled = p.scale*R_critical;
