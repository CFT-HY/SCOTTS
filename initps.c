...
{ 
  ...
  if(fabs(ksq) < 0.000001) {
    ...
    (*res)[0] = get_normal(0.0, exp(-sqrt(ksq)*((double)L)));
...
void init_ps(hydro_fields f, hydro_params p, double ***field) {
  ...
  */
  ...
  fftw_complex *in = fftw_alloc_complex(p.Lx*p.Ly*p.Lz);
  fftw_complex *out = fftw_alloc_complex(p.Lx*p.Ly*p.Lz);
  ...
  for(x=0;x<p.Lx;x++) {
	...
	spectrum(ksq, p, &(in[x*p.Ly*p.Lz + y*p.Lz + z]));
      }
    }
  }
...
//  fprintf(stderr,"x_thickness is %d\n", x_thickness);
  */
  ...
  for(x=0;x<p.Lx;x++) {
    for(y=0;y<p.Ly;y++) {
      for(z=0;z<p.Lz;z++) {
	in[x*p.Ly*p.Lz + y*p.Lz + z][0] = in[((p.Lx-x)%p.Lx)*p.Ly*p.Lz
						  + ((p.Ly-y)%p.Ly)*p.Lz + ((p.Lz-z)%p.Lz)][0];
	in[x*p.Ly*p.Lz + y*p.Lz + z][1] = -1.0*in[((p.Lx-x)%p.Lx)*p.Ly*p.Lz
						       + ((p.Ly-y)%p.Ly)*p.Lz + ((p.Lz-z)%p.Lz)][1];

	//	true_x = x;

	if((x == 0 || x == p.Lx/2) && (y == 0 || y == p.Ly/2) && (z == 0 || z == p.Lz/2)) {
	  in[x*p.Ly*p.Lz + y*p.Lz + z][0] = sqrt(in[((p.Lx-x)%p.Lx)*p.Ly*p.Lz
						    + ((p.Ly-y)%p.Ly)*p.Lz + ((p.Lz-z)%p.Lz)][0]
						 *in[((p.Lx-x)%p.Lx)*p.Ly*p.Lz
						     + ((p.Ly-y)%p.Ly)*p.Lz + ((p.Lz-z)%p.Lz)][0]
						 + in[((p.Lx-x)%p.Lx)*p.Ly*p.Lz
						      + ((p.Ly-y)%p.Ly)*p.Lz + ((p.Lz-z)%p.Lz)][1]
						 *in[((p.Lx-x)%p.Lx)*p.Ly*p.Lz
  ...
  for(x=1;x<=p.slicex;x++) {
    for(y=1;y<=p.slicey;y++) {
      for(z=0;z<p.Lz;z++) {	
	field[x][y][z] = out[(x+p.shiftx-1)*p.Ly*p.Lz + (y+p.shifty-1)*p.Lz + z][0];
