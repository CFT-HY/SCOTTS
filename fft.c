	...
	for(i=1; i<=3; i++) {
	  for(j=1; j<=3; j++) {
		...
		product[x*p.Ly*p.Lz + y*p.Lz + z] +=
		  ...
		  *
		  (udot[indexof(i,j)][x*p.Ly*p.Lz + y*p.Lz + z][0]*udot[indexof(l,m)][x*p.Ly*p.Lz + y*p.Lz + z][0]
		   + udot[indexof(i,j)][x*p.Ly*p.Lz + y*p.Lz + z][1]*udot[indexof(l,m)][x*p.Ly*p.Lz + y*p.Lz + z][1]);
	      }
	    }
	  }
	}
