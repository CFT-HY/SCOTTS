  ...
  for(x = 1; x <= p.slicex; x++) {
	...
	f.pi[x][y][z] = f.pifull[x][y][z] - 0.5*p.dt
	  *(f.phi[x+1][y][z] + f.phi[x][y+1][z] + f.phi[x][y][((z+1)%p.Lz)] 
	    - 6.0*f.phi[x][y][z] 
	    + f.phi[x-1][y][z] + f.phi[x][y-1][z] + f.phi[x][y][((z-1+p.Lz)%p.Lz)])/(p.dx*p.dx)
	  + 0.125*p.dt*p.dt
	  *(f.pifull[x+1][y][z] - 2.0*f.pifull[x][y][z] + f.pifull[x-1][y][z])/(p.dx*p.dx)
...
void evolve_field(hydro_fields f, hydro_params p) {
  ...
  for(x = 1; x <= p.slicex; x++) {
    ...
    f.pi[x][y][z] = f.pi[x][y][z] - (
			 + 0.5*p.dt*p.C*f.W[x][y][z]*(f.V[0][x][y][z]*(f.phi[x][y][z] - f.phi[x-1][y][z])
						+ f.V[0][x+1][y][z]*(f.phi[x+1][y][z] - f.phi[x][y][z]))/p.dx
			 + 0.5*p.dt*p.C*f.W[x][y][z]*(f.V[1][x][y][z]*(f.phi[x][y][z] - f.phi[x][y-1][z])
						      + f.V[1][x][y+1][z]*(f.phi[x][y+1][z] - f.phi[x][y][z]))/p.dx
...
void evolve_hydro(hydro_fields f, hydro_params p) {
  ...
  for(x = 1; x <= p.slicex; x++) {
    ...
    dxphi[0][x][y][z] =  0.5*(f.phiold[x+1][y][z] + f.phi[x+1][y][z]
			- f.phiold[x][y][z] - f.phi[x][y][z])/p.dx;

    dxphi[1][x][y][z] =  0.5*(f.phiold[x][y+1][z] + f.phi[x][y+1][z]
			- f.phiold[x][y][z] - f.phi[x][y][z])/p.dx;
  ...
  for(x = 1; x <= p.slicex; x++) {
    ...
    double vdnb = 0.125*(Vdmid[x-1][y][z] + Vdmid[x][y][z] + Vdmid[x][y-1][z] + Vdmid[x-1][y-1][z]
			 + Vdmid[x][y][((z-1+p.Lz)%p.Lz)] + Vdmid[x][y-1][((z+p.Lz-1)%p.Lz)] + Vdmid[x-1][y][((z+p.Lz-1)%p.Lz)] + Vdmid[x-1][y-1][((z+p.Lz-1)%p.Lz)]);


    double pinb = 0.125*(f.pi[x-1][y][z] + f.pi[x][y][z] + f.pi[x][y-1][z] + f.pi[x-1][y-1][z]
			 + f.pi[x][y][((z-1+p.Lz)%p.Lz)] + f.pi[x][y-1][((z+p.Lz-1)%p.Lz)] + f.pi[x-1][y][((z+p.Lz-1)%p.Lz)] + f.pi[x-1][y-1][((z+p.Lz-1)%p.Lz)]);


    double wnb = 0.125*(f.W[x-1][y][z] + f.W[x][y][z] + f.W[x][y-1][z] + f.W[x-1][y-1][z]
			 + f.W[x][y][((z-1+p.Lz)%p.Lz)] + f.W[x][y-1][((z+p.Lz-1)%p.Lz)] + f.W[x-1][y][((z+p.Lz-1)%p.Lz)] + f.W[x-1][y-1][((z+p.Lz-1)%p.Lz)]);

   
    double dxphinb0 = 0.25*(dxphi[0][x-1][y][z] + dxphi[0][x-1][y-1][z]
			   + dxphi[0][x-1][y][((z+p.Lz-1)%p.Lz)] + dxphi[0][x-1][y-1][((z+p.Lz-1)%p.Lz)]);


    double dxphinb1 = 0.25*(dxphi[1][x][y-1][z] + dxphi[1][x-1][y-1][z]
			   + dxphi[1][x][y-1][((z+p.Lz-1)%p.Lz)] + dxphi[1][x-1][y-1][((z+p.Lz-1)%p.Lz)]);

    double dxphinb2 = 0.25*(dxphi[2][x][y][((z-1+p.Lz)%p.Lz)] + dxphi[2][x-1][y][((z+p.Lz-1)%p.Lz)]
			   + dxphi[2][x][y-1][((z+p.Lz-1)%p.Lz)] + dxphi[2][x-1][y-1][((z+p.Lz-1)%p.Lz)]);
    ...
    gpi = f.W[x][y][z]*(f.pi[x][y][z] + 0.5*(f.V[0][x][y][z]*dxphi[0][x-1][y][z]
				 + f.V[0][x+1][y][z]*dxphi[0][x][y][z]));
    f.E[x][y][z] = f.E[x][y][z] + p.dt*(p.C*gpi*gpi + gpi*Vdmid[x][y][z]);


    gpi = f.W[x][y][z]*(f.pi[x][y][z] + 0.5*(f.V[1][x][y][z]*dxphi[1][x][y-1][z]
				 + f.V[1][x][y+1][z]*dxphi[1][x][y][z]));
    f.E[x][y][z] = f.E[x][y][z] + p.dt*(p.C*gpi*gpi + gpi*Vdmid[x][y][z]);
  ...
  for(x = 1; x <= p.slicex; x++) {
    ...
    */
    ...
    sigmabar = (f.kappa[x][y][z]*f.E[x][y][z] 
		...
		+ f.kappa[x-1][y-1][((z+p.Lz-1)%p.Lz)]*f.E[x-1][y-1][((z+p.Lz-1)%p.Lz)]
		+ f.kappa[x][y-1][((z+p.Lz-1)%p.Lz)]*f.E[x][y-1][((z+p.Lz-1)%p.Lz)]
		+ f.kappa[x-1][y][((z+p.Lz-1)%p.Lz)]*f.E[x-1][y][((z+p.Lz-1)%p.Lz)]
	...
	for(z = 0; z < p.Lz; z++) {
	  f.udotij[i][x][y][z] = f.udotij[i][x][y][z] + p.dt*
	    (f.uij[i][x+1][y][z] + f.uij[i][x][y+1][z] + f.uij[i][x][y][((z+1)%p.Lz)]
	     - 6.0*f.uij[i][x][y][z]
	     + f.uij[i][x-1][y][z] + f.uij[i][x][y-1][z] + f.uij[i][x][y][((z-1+p.Lz)%p.Lz)])
	    /(p.dx*p.dx);
